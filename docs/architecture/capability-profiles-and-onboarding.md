...
## Hosted Babo offerings (optional)

When you operate a hosted control plane (`BABO_CLOUD_MODE=true`), Nest can relay inference and GPU workers to upstream URLs you configure. OSS users typically self-host Nest on Railway or run desktop-only with BYOK inference.

See [Cloud deployment](../configuration/cloud-deployment.md) and [NestJS Babo Cloud module](nestjs-modules/babo-cloud.md).
...
| Item | Status |
|------|--------|
| Capability schema + TS types | `nls/config/capability-profile.schema.json`, `desktop/electron/capability-types.ts` |
| Device scan + LAN probe + recommend | `capability-scanner.ts`, `capability-recommender.ts`, IPC `capabilities:*` |
| `getRuntimeEnv()` maps `gpuWorkerUrl`, placements | Shipped — `config-manager.ts` |
| Setup wizard workload cards | Shipped — 10-step capability flow in desktop shell |
| `VisualCortex(..., gpu_worker_url=)` from env | Partial — see `factory.py` + profile JSON |
