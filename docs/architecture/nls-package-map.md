...
Top-level modules (not packages):

| Module | Path | Responsibility |
|--------|------|----------------|
| **platform_shell** | `nls/platform_shell.py` | Resolve `bash()` shell: bundled `pwsh`, PATH `pwsh`, or Windows PowerShell 5 |
| **skills_setup_policy** | `nls/skills_setup_policy.py` | Instruction-skill activation checklist; Windows/Python-first setup nudges |
