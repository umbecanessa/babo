...
## Config written at end of onboarding
...
1. ~~**Scan IPC**~~ — shipped: `capabilities:scan-device`, `capabilities:probe-lan`.  
2. ~~**Capabilities step UI**~~ — shipped: four cards, experience strip, `capabilities:apply-profile`.  
3. ~~**Wire profile → env**~~ — shipped: `config-manager.getRuntimeEnv()`, `factory.py`.  
4. ~~**Ambient toggle**~~ — shipped: default off; `capabilities:prefetch-vision` on enable only.  
