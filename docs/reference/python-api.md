...
| `/agents/{id}/skills/` | Per-agent enable, config, onboarding, repair |
...
| Method | Path | Purpose |
|--------|------|---------|
| POST | `/webhooks/*` | Legacy direct ingress (`routes/webhooks.py`) |
| POST | `/skills/*-channel/webhook/{id}` | Skill bridges (WhatsApp, etc.) |
| GET | `/channels/*` | Channel status (`routes/channels.py`) |
...
| Method | Path | Purpose |
|--------|------|---------|
| POST | `/transcribe` | Audio → text (local Whisper or GPU worker proxy) |
